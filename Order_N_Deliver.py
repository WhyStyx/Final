...
# Gets customer info from first window and displays it on the second window
...
# pizza Labels
    pepperoni_title = Label(left_frame, text="Pepperoni Pizza: $8", font=('SERIF', 14))
    pepperoni_title.place(x=5, y=65)
    supreme_pizza_title = Label(left_frame, text="Supreme Pizza: $12", font=('SERIF', 14))
    supreme_pizza_title.place(x=205, y=65)
    meats_title = Label(left_frame, text="Meats Pizza: $12", font=('SERIF', 14))
    meats_title.place(x=425, y=65)
    veggie_title = Label(left_frame, text="Veggie Pizza: $10", font=('SERIF', 14))
    veggie_title.place(x=5, y=250)
    buff_chicken_title = Label(left_frame, text="Buffalo Chicken Pizza: $12", font=('SERIF', 14))
    buff_chicken_title.place(x=185, y=250)
    taco_pizza_title = Label(left_frame, text="Taco Pizza: $10", font=('SERIF', 14))
    taco_pizza_title.place(x=455, y=250)

# Add Pizzas, spin boxes are used to add or subtract pizzas from the order
    pep_spin = Spinbox(left_frame, from_=0, to=999) # pepperoni pizza spinbox
    pep_spin.place(x=45, y=165)
    supreme_spin = Spinbox(left_frame, from_=0, to=999) # supreme pizza spinbox
    supreme_spin.place(x=230, y=165)
    meats_spin = Spinbox(left_frame, from_=0, to=999) # Meats pizza spinbox
    meats_spin.place(x=440, y=165)
    veggie_spin = Spinbox(left_frame, from_=0, to=999) # veggie pizza spin box
    veggie_spin.place(x=45, y=365)
    buff_chicken_spin = Spinbox(left_frame, from_=0, to=999) # buffalo chicken pizza spin box
    buff_chicken_spin.place(x=230, y=365)
    taco_spin = Spinbox(left_frame, from_=0, to=999) # taco Pizza spin box
    taco_spin.place(x=440, y=365)

# Function gets values of spin boxes and adds them to calculate total
    def add_pizza():
...
# Function submits the entered delivery and/or carry-out information to the database
def click_submit():
...
# Create cursor
    ...
    # Clear the Text Boxes when submit button is clicked
    f_name.delete(0, END)
    l_name.delete(0, END)
...
def Click_Delivery():
    ...
    global reset_btn
    global btn
# Create text boxes to enter customer info for the Delivery option
...
# Create Text Box Labels for the carry-out option
...
# button that opens the second window
...
# reset button to destroy(close) all labels and entry boxes for Delivery
...
def click_carryout():
    ...
    global b_submit
    global reset_btn
    # Create text boxes to enter customer info for the Carry_Out option
    f_name = Entry(bot_frame, width=30)
    f_name.grid(row=0, column=1)
    l_name = Entry(bot_frame, width=30)
    l_name.grid(row=1, column=1)
    ...
    phone.grid(row=2, column=1)
    address = Entry(bot_frame, width=30)
# placeholder entry box to be used in carryout for the database, but not placed on the screen
    city = Entry(bot_frame, width=30)
# placeholder entry box to be used in carryout for the database, but not placed on the screen
    state = Entry(bot_frame, width=30)
# placeholder entry box to be used in carryout for the database, but not placed on the screen
    ...
    # Create Text Box Labels for the Carry_Out option
    f_name_label = Label(bot_frame, text="Enter first name")
    f_name_label.grid(row=0, column=0)
    l_name_label = Label(bot_frame, text="Enter last name")
    l_name_label.grid(row=1, column=0)
...
# button that opens the second window
    btn = Button(bot_frame, text='Continue to next screen', command=lambda: OrderingScreen())
    btn.grid(row=8, column=0, columnspan=2, pady=10, padx=10, ipadx=137)
# reset button to destroy(close) all labels and entry boxes for carry out
...
# reset button function to destroy(close) all labels and entry boxes for delivery
def reset():
    f_name.destroy()
    l_name.destroy()
    address.destroy()
